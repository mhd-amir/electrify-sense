...
import { Boxes, Box, LayoutGrid, Maximize2 } from "lucide-react";
  ...
  head: () => ({
    meta: [
      { title: "Digital Twin Network — GridTwin" },
      {
        name: "description",
        content:
          "Interactive digital twin of the national transmission network with live power-flow animation on every corridor.",
      },
      { property: "og:title", content: "Digital Twin Network — GridTwin" },
      {
        property: "og:description",
        content:
          "Zoom, pan and inspect live generation, substation and load assets on the grid twin.",
      },
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: selected
          ? `0 0 0 2px ${color}, 0 0 26px -4px ${color}`
          : `0 0 22px -10px ${color}`,
      }}
...
function Canvas() {
  ...
  const onEdgeClick = useCallback(
    (_: unknown, edge: Edge) => setSelectedId(edge.id),
    [setSelectedId],
  );
    ...
    <div className="relative h-[calc(100vh-16rem)] min-h-[520px] overflow-hidden rounded-2xl border border-border/60">
      ...
      >
        ...
        <MiniMap
          ...
          style={{ background: "rgba(16,24,39,0.85)" }}
        />
...
function ViewModeToggle({ mode, onChange }: { mode: ViewMode; onChange: (m: ViewMode) => void }) {
  return (
    <div className="flex items-center gap-1 rounded-xl border border-border/60 bg-panel-2/60 p-1">
      <button
        onClick={() => onChange("2d")}
        className={`inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-[11px] font-semibold tracking-wider uppercase transition-colors ${
          mode === "2d" ? "bg-info/15 text-info" : "text-muted-foreground hover:text-foreground"
        }`}
      >
      ...
      <button
        onClick={() => onChange("3d")}
        className={`inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-[11px] font-semibold tracking-wider uppercase transition-colors ${
          mode === "3d" ? "bg-info/15 text-info" : "text-muted-foreground hover:text-foreground"
        }`}
      >
  ...
  return (
    <Panel>
      <PanelHeader
        ...
        subtitle={
          mode === "2d"
            ? "Drag to pan, scroll to zoom, click any asset or corridor for live telemetry"
            : "Drag to orbit, scroll to zoom, click any 3D asset or corridor for live telemetry"
        }
        right={
          <div className="flex items-center gap-3">
            <div className="tabular hidden gap-4 text-[11px] text-muted-foreground sm:flex">
              ...
              <span className="text-crit">
                {state.lines.filter((l) => l.status === "failed").length} tripped
              </span>
            </div>
      ...
      {mode === "2d" ? (
        ...
        <ClientOnly
          fallback={
            <div className="flex h-[calc(100vh-16rem)] min-h-[520px] items-center justify-center rounded-2xl border border-border/60 text-sm text-muted-foreground">
              Loading 3D scene…
            </div>
          }
        >
